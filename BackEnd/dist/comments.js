...
export default function comments(app) {
    app.get('/comments/:id', async (req, res) => {
        let query = 'SELECT `users`.`firstName`, `users`.`lastName`, `comments`.* FROM `users`, `comments` WHERE `publicationId` = ? AND `comments`.`userId` = `users`.`id` ORDER BY `comments`.`id` DESC;';
    ...
    app.post('/comment', async (req, res) => {
        ...
        let query = 'INSERT INTO `comments` (`id`, `userId`, `publicationId`, `text`) VALUES (?, ?, ?, ?)';
