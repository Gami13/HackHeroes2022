...
export default function reminders(app) {
    app.post('/getReminders', async (req, res) => {
        ...
        if (results.length == 0)
            return res.sendError('No results');
    ...
    app.post('/reminder', async (req, res) => {
        ...
        let [results] = await db.query(query, [title, date]);
    ...
    app.post('/reminders', async (req, res) => {
        ...
        let [results] = await db.query(q, [userId]);
        if (results.length > 0) {
            ...
            results.forEach(async (reminder) => {
                ...
                if (!found) {
                    ...
                    await db.query(query, [reminder.id]);
                }
            });
        }
        ...
        for (let i = 0; i < left.length; i++) {
            let query = 'INSERT INTO `reminders` (`title`, `date`,`userId`) VALUES (?,?,?)';
            let [results] = await db.query(query, [
                left[i].title,
